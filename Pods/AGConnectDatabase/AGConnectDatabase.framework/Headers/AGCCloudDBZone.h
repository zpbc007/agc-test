...
/**
 A block on server status callback has two parameters: server status, error, use it when need.
 
 @param serverStatus Queryed server status object.
 @param error Error message, use this when need.
 */
typedef void (^AGCServerStatusCallback) (AGCServerStatus *_Nullable serverStatus, NSError *_Nullable error);
...
#pragma mark - query server status

/**
 Developers use this API to query server status.
 
 @param callback Callback return the result of query.
 */
- (void)executeServerStatusQueryOnCompleted:(nullable AGCServerStatusCallback)callback;
